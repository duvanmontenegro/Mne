...
fig=raw.plot(order=mag_channels, proj=False,n_channels=len(mag_channels), remove_dc=False,title="Derivas lentas")
fig.subplots_adjust(top=0.9)
fig.suptitle('{} filtered'.format("Derivas lentas"), size='xx-large', weight='bold')
...
for cutoff in (0.1, 0.2):
	...
	fig = raw_highpass.plot(order=mag_channels, proj=False,n_channels=len(mag_channels), remove_dc=False,title="Pasa alta F")
...
print("|---------------|")
# print(raw.info['sfreq'])
print(raw.get_data()) #optener los datos
# filter_params = mne.filter.create_filter(raw.get_data(), 100,l_freq=0.2, h_freq=None)
...
figf=mne.viz.plot_filter(filter_params, raw.info['sfreq'], flim=(0.01, 5))
figf.subplots_adjust(top=0.9)
figf.suptitle('{} filtered'.format("Parametros f"), size='xx-large', weight='bold')
...
events = mne.find_events(raw_filtered, stim_channel="MNE_STIM_CHANNEL")
